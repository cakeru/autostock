...
function ReceiveDialog({ product, onClose, onConfirm, loading }: {
  product: Product
  onClose: () => void
  onConfirm: (data: { quantity: number; unit_cost?: number; supplier_id?: number; paid?: boolean; dot_code?: string; notes?: string; invoices?: { invoice_number?: string; invoice_image?: string; amount: number }[] }) => void
  loading: boolean
}) {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      onConfirm({
        ...
        invoices: valid.length > 0 ? valid : undefined,
      })
    } finally {
      ...
      <div className="bg-card rounded-lg p-5 shadow-lg w-full max-w-sm mx-4">
        ...
        <div className="space-y-2">
          ...
          <div className="space-y-1">
            ...
            <InvoiceEditor invoices={invoices} onChange={setInvoices} total={(parseFloat(cost) || 0) * (parseFloat(qty) || 0)} />
          </div>
          <div className="space-y-1">
