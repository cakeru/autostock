...
func (s *Service) ListVehicles(ctx context.Context, customerID int64) ([]models.Vehicle, error) {
	rows, err := s.pool.Query(ctx, `
		SELECT id, customer_id, plate_number, COALESCE(make,''), COALESCE(model,''), year, COALESCE(vin,''), COALESCE(color,''), COALESCE(body_type,''), COALESCE(notes,''), created_at, updated_at
...
func (s *Service) CreateVehicle(ctx context.Context, customerID int64, req *dto.CreateVehicleRequest) (*models.Vehicle, error) {
	...
	err := s.pool.QueryRow(ctx, `
		...
		RETURNING id, customer_id, plate_number, COALESCE(make,''), COALESCE(model,''), year, COALESCE(vin,''), COALESCE(color,''), COALESCE(body_type,''), COALESCE(notes,''), created_at, updated_at`,
