...
// and re-issuing a mis-keyed invoice can't double-count a service. lifeKm is
// the distance interval (tire wear life, or an oil product's change interval);
// lifeMonths is the oil product's time interval (nil for tires).
func LogEvent(ctx context.Context, tx pgx.Tx, branchID, vehicleID int64, eventType string, mileage *int, occurredAt time.Time, invoiceID, serviceJobID *int64, productName string, lifeKm, lifeMonths *int, createdBy *int64) error {
	_, err := tx.Exec(ctx, `
		INSERT INTO vehicle_service_events (branch_id, vehicle_id, event_type, mileage, occurred_at, invoice_id, service_job_id, product_name, life_km, life_months, created_by)
		...
		branchID, vehicleID, eventType, mileage, occurredAt, invoiceID, serviceJobID, productName, lifeKm, lifeMonths, createdBy)
	if err != nil {
